...
description: |
  触发：当工作完成、进入验收、收到批评反馈、发生生产事故，或同类错误需要系统纠偏时调用；常见信号包括 review、audit、retrospective、incident、quality check、纠错与复盘。
  English: Trigger after delivery, at a review checkpoint, or after a production incident when quality, impact, and methodological failures must be examined honestly and corrected permanently.
---
...
发生服务中断、用户影响或被迫手动恢复时，普通“做得好/做得不好”报告不够。必须记录：

- 用户影响、受影响组件/端口/能力，以及有证据支撑的开始、恢复时间和停机窗口。
- 最后可用基线、谁/什么动作使其失效、用户手动恢复了什么，以及恢复后的受保护 Runtime/PID。
- **确定性根因**、促成因素和仍未知项分栏书写；证据不足时不得把猜测写成根因。
- 为什么测试通过却没有发现部署风险：是否缺少 stop/start/cleanup/wait 失败注入、standby、回滚或真实链路复验。
- 应急止血、恢复证据、永久修复、可执行防复发门禁，以及每个后续项的状态/责任边界。

复盘以恢复和防复发为目的，但不能用“治病救人”淡化真实用户影响或方法论责任。
...
> 📌 **跟进闭环**：报告生成后，将“下次重点关注”直接输出给用户。任务可能跨压缩或会话时，按 `juzi-codex-continuity` 把未完成改进、受保护资源和唯一下一步写入项目 `.codex/ACTIVE_TASK.md`；没有该机制时再由用户保存并在下次提供报告。不得假设聊天记忆会自动形成执行闭环。
