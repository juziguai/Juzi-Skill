...
description: |
  触发：当你准备下判断、做决策或提出建议，但事实、一手信息或生产实时基线还不充分时优先调用；常见信号包括 unknowns、信息缺口、证据不足、领域陌生、服务状态不明。
  English: Trigger before making claims or decisions when context, firsthand evidence, or a live production baseline is incomplete. Use this skill to investigate first and let verified reality shape the conclusion.
---
...
- 紧急修复且问题现象已经明确 —— 可以跳过全面调查，但不能跳过生产最小基线、用户影响和回滚点记录
...
任务可能影响真实服务、端口、Runtime、系统配置或用户流量时，先建立一张**只读基线卡**。至少记录：

- 当前时间、用户影响和可确认的停机起点；无法确认时明确写“未知”。
- 服务状态、父子进程/PID、监听端口和轻量 Runtime/health API。
- 近期错误日志与一条真实业务流量证据；没有应用层可观测性时写“未观测”，不能写成错误率 `0`。
- 最后已知可用版本/Runtime、用户是否手动恢复、必须保护的进程、端口、数据或配置。
- 本轮允许的只读/可逆动作、禁止的破坏性动作、回滚点和救援入口。

基线采集本身不得停止服务、清理进程、轮转日志、压缩数据库或改变路由。发现用户手动恢复的 Runtime/PID 后，立即将其标为受保护资源，可靠替代者形成前不得操作。
