...
description: |
  触发：当多个任务或生产操作同时争夺时间、注意力、算力或变更窗口，必须确定主攻方向并停止分散用力时调用；常见信号包括优先级过多、资源紧张、事故恢复、重复重门禁或并行扰动。
  English: Trigger when limited resources or a production change window are split across too many tasks. Use this skill to protect the baseline, choose one main target, and stop parallel disturbance.
---
...
生产服务、共享端口、同一数据库或同一控制面上的动作不视为“完全独立”，即使它们修改不同文件。
...
事故或高风险切换时，主攻目标默认是**保持或恢复可用服务**，不是把原优化任务做完：

1. 冻结最后可用基线；用户手动恢复的 Runtime/PID 立即受保护。
2. 同一时刻只允许一个生产写动作；日志优化、清理、重构、第二套部署和等价重门禁全部暂缓。
3. `stop`、`restart`、`kill`、`cleanup` 失败一次后停止重试，回到 `investigation-first`；“集中兵力”不等于连续重复破坏性命令。
4. 普通迭代只跑定向回归、一次完整门禁和短时冒烟；重大升级/重构才增加 10 分钟快速验收。不得因为焦虑而重复运行等价的长门禁。
5. 只有服务恢复并通过真实链路验收后，才重新评估原优化任务。
...
1. **列出所有待处理任务**（使用宿主可用的计划工具或简洁清单），为每个任务标注：
...
3. **宣告并锁定**，格式：
   ...
   受保护资源：[不得操作的服务/进程/端口/数据]
   本轮禁止动作：[并行扰动、重复门禁或破坏性动作]
   重门禁预算：[普通一次 / 重大变更一次完整门禁 + 10 分钟快速验收]
