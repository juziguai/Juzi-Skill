...
description: |
  触发：当问题复杂、存在多个冲突因素、优先级不清，或生产变更目标与服务连续性冲突时调用；常见信号包括 trade-off、瓶颈、根因不明、主次不清、多个问题互相牵制。
  English: Trigger when a problem contains competing forces, unclear priorities, or tension between a production change and service continuity. Use this skill to identify the principal contradiction and choose the right response.
---
...
只要用户仍在依赖当前服务，先把 `[变更/修复目标] vs [服务连续性]` 列为候选主要矛盾。当前基线可用时，连续性是硬约束而不是普通权衡项：

- 替代者未通过 standby 就绪检查前，不得释放当前可用基线。
- 用户手动恢复的服务、Runtime 或 PID 自动成为受保护资源。
- 事故进行中，主要矛盾优先转为“恢复可用服务 vs 继续扩大变更”；先恢复，再优化。
- “测试通过”“脚本设计合理”不能证明切换安全；必须单独分析部署、权限、进程和回滚矛盾。
...
6. **生产场景增加边界**：写明受保护基线、替代者就绪条件，以及在这些条件满足前明确禁止的 stop/restart/kill/cleanup 动作。
